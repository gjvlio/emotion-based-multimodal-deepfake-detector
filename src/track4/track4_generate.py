...
track4_generate.py
==================
Track 4 deepfake generation: emotion-mismatch lip-sync via MuseTalk on MELD.
...
AND low-level lip/face artefacts (unlike a pure audio-swap baseline).
...
Contrast with other tracks:
  ...
  Track 4 — MELD, real cross-EMOTION audio, MuseTalk lips (this script)
...
Usage (run from repo root):
    ...
    # 10-batch run (~348 clips each, ~15 hrs/batch, ~150 hrs total):
    ...
    # ... continue in 348-clip increments until all 3,482 clips done
...
def main():
    parser = argparse.ArgumentParser(
        description="Track 4: emotion-mismatch MuseTalk lip-sync deepfakes from MELD"
    )
    parser.add_argument(
        '--pairs_csv',
    ...
    log.info("Starting Track 4 — MuseTalk emotion-mismatch lip-sync on MELD")
    log.info(f"Output: {out_dir}")
    log.info(f"MuseTalk: {musetalk_dir}  batch_size={args.batch_size}")

    for _, row in tqdm(pairs.iterrows(), total=len(pairs), desc="Track4-MuseTalk"):
        stem = row['output_stem']
        t0 = time.time()
        try:
